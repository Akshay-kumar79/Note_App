...
import com.akshaw.noteapp.feature_note.domain.use_cases.NoteUseCases
...
class NotesViewModel @Inject constructor(
    ...
    fun onEvent(event: NotesEvent) {
        when (event) {
            is NotesEvent.Order -> {
                if (state.value.noteOrderBy::class == event.noteOrderBy::class && state.value.noteOrderBy.orderType == event.noteOrderBy.orderType) {
                    return
                }
    ...
    private fun getNotes(noteOrderBy: NoteOrderBy) {
        getNotesJob?.cancel()
        getNotesJob = notesUseCases.getNotesUseCase(noteOrderBy).onEach { notes ->
            _state.value = state.value.copy(
                notes = notes,
